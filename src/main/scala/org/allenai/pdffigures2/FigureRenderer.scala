...
object FigureRenderer {
  ...
  def rasterizeFigures(
    ...
    clean: Boolean,
    logger: Option[VisualLogger],
    expandPercent: Double = 0.01  // 확장 비율 0.02 => 2퍼센트
  ): Seq[RasterizedFigure] = {
    val scale = dpi / 72.0
    val nonFigureContent =
      ((page.classifiedText.allText ++ page.paragraphs).map(_.boundary) ++
    ...
    val rasterized = page.figures.zipWithIndex.map {
      case (fig, figureNumber) =>
        ...
        // 너비와 높이를 구해 expandPercent로 확장
        val width = x2 - x1
        val height = y2 - y1
        val paddingX = (width * expandPercent).toInt  // 가로 2% 확장
        val paddingY = (height * expandPercent).toInt // 세로 2% 확장

        val (cx1, cy1, cx2, cy2) = if (clean) {
          expandFigureBounds(
            x1 - paddingX,  // 좌표를 패딩 만큼 확장
            y1 - paddingY,
            x2 + paddingX,
            y2 + paddingY,
            nonFigureContent ++ otherFigureRegions,
          ...
          (
            Math.max(x1 - paddingX - PadUnexpandedImage, 0),
            Math.max(y1 - paddingY - PadUnexpandedImage, 0),
            Math.min(x2 + paddingX + PadUnexpandedImage * 2, pageImg.getWidth - 1),
            Math.min(y2 + paddingY + PadUnexpandedImage * 2, pageImg.getHeight - 1)
          )
        }
