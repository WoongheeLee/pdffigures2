...
import java.io.{BufferedWriter, FileWriter} // woong
import org.apache.pdfbox.text.PDFTextStripper // woong
...
object FigureExtractorBatchCli extends Logging {
  ...
  case class CliConfigBatch(
    ...
    saveTextPrefix: Option[String] = None,  // To save extracted text file. woong
    saveTextDir: Option[String] = None,  // To save extracted text file, setting dest dir. woong
  )
  ...
  val Parser = new scopt.OptionParser[CliConfigBatch]("figure-extractor-batch") {
    ...
    // save json all captions and texts including meta data
    ...
    // to save text file. woong
    opt[String]('x', "text-dir") action { (d, c) => 
      c.copy(saveTextPrefix = Some(d))

    } text "Save the extracted text files in the specified directory '<text-dir>'"

    // to save text file. woong
    opt[String]('p', "text-prefix") action { (f, c) => 
      c.copy(saveTextPrefix = Some(f))
    } text "Save the extracted text to '<text-prefix><input_filename>.txt'"
  ...
  def processFile(
    ...
    try {
      ...
      // Extract and save the text if saveTextPrefix is defined. woong
      if (config.saveTextPrefix.isDefined) {
        val outputTextFile = new File(s"${config.saveTextPrefix.get}$truncatedName.txt")
        val textStripper = new PDFTextStripper()
        val textContent = textStripper.getText(doc)
        val writer = new BufferedWriter(new FileWriter(outputTextFile))
        try {
          writer.write(textContent)
        } finally {
          writer.close()
        }
        logger.info(s"Text extracted and saved to ${outputTextFile.getAbsolutePath}")
      }
